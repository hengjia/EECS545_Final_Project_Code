...
[alpha,t]=newton(gamma_A,gamma_I,Y,K,L,alpha);

alpha_num=find(alpha~=0);
test_ans=sign(K(:,alpha_num)*alpha);
test_ans = test_ans';
...
%}
function [alpha,t] = newton(gamma_A,gamma_I,Y,K,L,alpha)
% {newton} trains the classifier using the Newton's method.
    ...
    while 1  
        ...
        if gamma_I==0 % SVM (sparse solution)
            alpha_new=zeros(n,1);
            alpha_new(sv)=(gamma_A*speye(nsv)+IsvK(sv,sv))\Y(sv);
